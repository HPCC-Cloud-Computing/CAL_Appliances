...
def connect_to_system(db_setting_modules, SYSTEM_INFO):
    ...
    if len(SystemCluster.objects.filter(
        ...
        #     print(e.message)
        #     raise SystemConnectionError(e.message)  # local setup for first cluster in system
...
def local_setup(SystemCluster, ObjectServiceInfo):
    ...
    if len(SystemCluster.objects.filter(
        ...
        service_info = ObjectServiceInfo.create_service_info_data(
            ...
            json.dumps(auth_info), json.dumps(service_specs))
        service_info.save()
        ...
        # add cluster info to shared database
        shared_db_connect = SharedDatabaseConnection()
        shared_db_connect.add_cluster_to_shared_database(
            current_cluster_id, MCOS_CLUSTER_NAME, MCOS_IP, MCOS_PORT
        )
...
def remote_setup(connect_server, SystemCluster, ObjectServiceInfo):
    ...
    service_info = json.dumps({
        'id': str(uuid.uuid4()),
        'type': service_type,
        'specifications': json.dumps(service_specs),
        ...
        while retry_connect:
            ...
            if status_code == 200:
                ...
                if resp_data['is_connected_to_system'] == 'true':
                    ...
                    shared_db_connect = SharedDatabaseConnection()
                    shared_db_connect.add_cluster_to_shared_database(
                        cluster_id, cluster_name, address_ip,
                        address_port
                    )
                    return cluster_id
                else:
...
def create_storage_container(service_type, access_info):
    ...
    storage_container_exist = check_container_is_exist(
        service_connector,
        STORAGE_CONTAINER_NAME)  # only create service container if not exist
...
def add_cluster_info_to_shared_database():
    pass
